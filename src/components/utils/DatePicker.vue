...
const props = withDefaults(
  ...
  }>(),
  {
    range: (function () {
      const d = new Date();
      return `${d.getFullYear()}-${d.getFullYear() + 10}`;
    })(),
  }
);
...
const yearArray = computed(() => {
  return getYearList(props.range);
});
...
const firstDayOfMonth = computed(() => {
  const date = new Date(
    yearArray.value[selectedYearIndex.value],
...
function prepareDate() {
  const dateString = `${yearArray.value[selectedYearIndex.value]}-${
...
<template>
  <div class="date-picker">
    ...
    <transition name="calandar">
      <div v-if="open" class="calandar">
        <p class="deadline">Need to finish by</p>
        ...
        <div class="month">
          ...
          <div class="day-nums">
            <div
              ...
              :key="i"
              class="day"
              :class="i == selectedDay + firstDayOfMonth ? 'selected' : ''"
              @click="
                () => {
            ...
            >
              {{
                getDateRange[0] <= i - firstDayOfMonth &&
                i - firstDayOfMonth <= getDateRange[1]
                  ? i - firstDayOfMonth
                  : ' '
              }}
            </div>
        ...
        <IconClose class="close-btn" @click="toggleOpen(false)" /></div
    ></transition>
  </div>
</template>
...
.calandar-enter-active,
.calandar-leave-active {
